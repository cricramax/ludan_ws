...
private:
  ...
  // command_moveJ 对应的关节索引（可通过参数覆盖）
  std::vector<size_t> movej_indices_;

  // 命令缓存：N x 5 (pos, vel, kp, kd, ff)
  std::vector<std::array<double,5>> cmd_;

  // 默认参数
  double default_kp_{10.0}, default_kd_{1.0}, default_vel_{0.0}, default_ff_{0.0};

  bool initial_position_latched_{false};
  bool flip_sign_{false};
  // 订阅者
