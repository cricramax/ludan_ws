...
void DmHW::read(const ros::Time & /*time*/, const ros::Duration & /*period*/)
{
  ...
  for (int i=0; i< NUM_JOINTS; ++i)
  {
    ...
    ROS_INFO_STREAM_THROTTLE(0.5, "pos[" << i << "] = " << jointData_[i].pos_);

    if (hold_position_mode_.load())
    {
  ...
  if (!received_first_state_.load())
  {
    ...
    if (!hold_notice_printed_.load())
    {
      ROS_INFO("[DmHW] 首次读取到电机状态，保持当前姿态待命。");
      hold_notice_printed_.store(true);
    }
  }
...
void DmHW::write(const ros::Time& time, const ros::Duration& /*period*/)
{
  ...
  for (int i = 0; i < NUM_JOINTS; ++i)
  {
    ...
    else
    {
      ...
      kd      = jointData_[i].kd_;
      ff      = jointData_[i].ff_;
      // ROS_INFO_STREAM("not manual!!!!");
    }

    if (hold_position_mode_.load())
    {
      const bool command_is_hold =
          std::fabs(pos_des - jointData_[i].pos_) <= hold_position_tolerance_ &&
          std::fabs(vel_des) <= hold_position_tolerance_ &&
          std::fabs(ff) <= hold_position_tolerance_;

      if (command_is_hold)
      {
        hold_position_synced_.store(true);
      }

      if (!command_is_hold && hold_position_synced_.load())
      {
        hold_position_mode_.store(false);
        hold_position_synced_.store(false);
        if (!hold_release_printed_.load())
        {
          ROS_INFO("[DmHW] 收到首个运动指令，退出保持模式。");
          hold_release_printed_.store(true);
        }
      }
      else
      {
        pos_des = jointData_[i].pos_;
        vel_des = 0.0;
        kp      = 0.0;
        kd      = 0.0;
        ff      = 0.0;
  ...
  // ROS_INFO_STREAM("cmd_pos = " << a.data[0] << ", " << a.data[1] << ", ...");
}
