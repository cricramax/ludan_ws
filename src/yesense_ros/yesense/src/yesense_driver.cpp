    ...
    , configured_(false)
{
    ...
    euler_pub_ = nh.advertise<std_msgs::Float64MultiArray>("yesense/imu", 9);
    // initSerial();
...
void YesenseDriver::publish_imu(const protocol_info_t &imu_data)
{
    ...
    std_msgs::Float64MultiArray euler_msg;
    euler_msg.data.resize(9);
    euler_msg.data[0]=imu_data.angle_x / 180.0 * M_PI;
    euler_msg.data[1]=imu_data.angle_y / 180.0 * M_PI;
    euler_msg.data[2]=imu_data.angle_z / 180.0 * M_PI;
    euler_msg.data[3]=imu_data.accel_x;
    euler_msg.data[4]=imu_data.accel_y;
    euler_msg.data[5]=imu_data.accel_z;
    euler_msg.data[6]=imu_data.roll / 180.0 * M_PI;
    euler_msg.data[7]=imu_data.pitch / 180.0 * M_PI;
    euler_msg.data[8]=imu_data.yaw / 180.0 * M_PI;
