...
void LeggedHWLoop::update()
{
  ...
  const auto lastTime2 = Clock::now();
  hardwareInterface_->write(ros::Time::now(), elapsedTime_);

  const auto currentTime2 = Clock::now();
  Duration time_span2 = std::chrono::duration_cast<Duration>(currentTime2 - lastTime2);
  elapsedTime2= ros::Duration(time_span2.count());
 // ROS_INFO( "MY_cycle time: %f" ,elapsedTime2 );
